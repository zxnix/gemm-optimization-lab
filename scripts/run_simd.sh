...
result_root=${1:-results/phase1/simd}
sizes=${GEMM_SIZES:-256,512,1024}
repeats=${GEMM_REPEATS:-7}
block_sizes=${GEMM_BLOCK_SIZES:-64,128}
...
if ! grep -qwm1 avx2 /proc/cpuinfo || ! grep -qwm1 fma /proc/cpuinfo; then
    echo "error: Phase 1.6 requires an x86 CPU with AVX2 and FMA" >&2
    exit 1
fi
...
for block_size in "${sizes_array[@]}"; do
    for kernel in packed micro avx2; do
